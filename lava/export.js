...
const provider = new JsonRpcProvider("https://eth1.lava.build/lava-referer-c8410a52-8703-4da7-9c53-85acd0427b35/")
...
async function exportRpc() {
    ...
    for (privateKey of privateKeyArr) {
        try {
            await appendRpc(privateKey, rpcPool)
        } catch(err) {
...
async function appendRpc(privateKey, rpcPool) {
    ...
    // request 3 获取RPC数据
    const loginVerifyCookie = loginVerifyRes.headers.get('set-cookie').replaceAll("Secure,", "");
